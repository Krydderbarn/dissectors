-- sci-cc/messages/status/diamond_crossing_status.lua
-- SCI-CC Status: Diamond Crossing Status
--
-- Message Type:       0x0040
-- Information Type:   0x08
-- Source: Eu.Doc.50 v4.3 (0.A), §3.5.6.17
--
...
return function (buf, tree)
    ...
    ----------------------------------------------------------------
    ids.add(tree, F.diamond_crossing_id, buf:range(44,20))

    ----------------------------------------------------------------
    -- Core state
    ----------------------------------------------------------------
    tree:add(F.active_control,           buf:range(64,1))
    tree:add(F.diamond_direction,        buf:range(65,1))
    tree:add(F.diamond_used_locked,      buf:range(66,1))
    tree:add(F.diamond_flank_protection, buf:range(67,1))
    ...
    ----------------------------------------------------------------
    -- EC Route Blocking (bytes 93..98)
    ----------------------------------------------------------------
    if pktlen >= 99 then
        decode_ec_blocking(buf, tree, 93)
    end
